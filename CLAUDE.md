...
3. **做完任务记得关掉对应的 issue**：任务对应某个 GitHub issue 且已完工（合并推送后），用 `gh issue close <编号> -c "<一句话说明怎么完成的>"` 关掉；顺手扫一眼其余 open issue，凡现状已被后续工作完成/超越的（如某省已接入、某 wontfix 前提已不成立）一并关掉并注明原因，别让已完成的单子挂着。
...
| 浙江 `zj` | 分数/计划/一分一段 | `各省份/浙江/浙江/…`（22-25 合表 + 一分一段，与重庆/山东同规范；自 ADR-0022 起换用一致源、走通用 `major` 模型，退役旧 `09、浙江` 万师兄专属栈；院校属性改按校名挂全国表，不再有按码「一表联动」） |

`hlj` 的源根由 `defaultSrc()`=`~/Downloads/高考志愿` 给出（`import_hlj.go`），通用省（含浙江）的
...
浙江原走专属 `import_zj.go`+`09、浙江` 源，2026-07-02 随 ADR-0022 归一到 `各省份/浙江`。）
