...
import type { SchoolIndexEntry, TrackRange } from "../lib/data";

const fmt = (n?: number) => (n ? n.toLocaleString("en-US") : "");
const rangeOf = (s: SchoolIndexEntry, track: string): TrackRange | undefined => s.ranges?.[track];

// 院校层次角标：985 ⊃ 211 ⊃ 双一流，取最高一档显示。
function tier(s: SchoolIndexEntry): { label: string; cls: string } | null {
...
function TrackLine({ track, r, active }: { track: string; r?: TrackRange; active: boolean }) {
  if (!r) return null;
...
/** 按院校名/代码站内搜索 + 按科类录取分排序，每校各科类录取线区间分开列。 */
export default function SchoolSearch({ schools, prov }: { schools: SchoolIndexEntry[]; prov: string }) {
  const cfg = provinceConfig(prov);
  const trackNames = cfg.tracks.map((t) => t.name);
  const multiTrack = trackNames.length > 1;
  const LS_KEY = `yuanxiao.sortTrack.${prov}`;

  const [q, setQ] = useState("");
  const [sortTrack, setSortTrack] = useState<string>(trackNames[0]);

  const [ready, setReady] = useState(false);
  useEffect(() => {
    try {
      const v = localStorage.getItem(LS_KEY);
      if (v && trackNames.includes(v)) setSortTrack(v);
    } catch {
      /* 忽略 */
    }
    setReady(true);
  ...
  const list = useMemo(() => {
    ...
    const key = (s: SchoolIndexEntry) => rangeOf(s, sortTrack)?.maxScore ?? -1;
          ...
          <li>
            ...
            >
              ...
              <div class="mt-1 space-y-0.5 text-xs text-slate-500">
                {trackNames.map((t) => (
                  <TrackLine track={t} r={rangeOf(s, t)} active={sortTrack === t} />
                ))}
                {trackNames.every((t) => !rangeOf(s, t)) && <div class="text-slate-300">暂无录取线数据</div>}
              </div>
            </a>
          </li>
        ))}
