// Package hlj 是黑龙江高考数据的领域逻辑：一分一段、位次换算、等效位次、
// 专业录取分数线挂接、选科判定。这些是全站正确性风险集中处，均为可表驱动测试
// 的纯函数 / 纯解析。见 docs/adr/0005。
package hlj
...
// ParseYiFenYiDuanXLSX 解析官方一分一段 xlsx。
// 布局：首行大标题，可能有空行，随后一行表头含"累计"，其后为数据行。
