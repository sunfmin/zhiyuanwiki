...
// 12 学科门类 → 1 字码。其他归「他」。客户端 src/lib/filters.ts 有镜像表，改这里要同步。
// 见 ADR-0008（专业大类 = 12 学科门类，自建 专业名→门类 映射）。
...
// 先用从旧表学到的「专业名→门类」精确映射，再退到关键词启发式。见 ADR-0008。
...
// LoadMenlei 从旧格式表（含「门类」+「专业」列）学习 专业名→门类 精确映射。
// 只有少数旧科类表带门类列，无该列的文件静默跳过。
