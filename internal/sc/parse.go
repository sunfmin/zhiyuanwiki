// Package sc 是四川高考数据的省份专属解析（3+1+2：物理/历史 + 院校专业组）。
...
// ParseScores 解析四川「专业录取分数」xlsx → 行表（仅物理/历史本科、含最低位次）。表头驱动。
...
// ParsePlan 解析四川「招生计划」xlsx → 计划行（仅物理/历史本科）。表头驱动。GroupCode 取所属专业组。
...
func parsePlan(s *core.Sheet) []core.PlanRow {
	...
	for _, r := range s.Data {
		...
		// 四川招生计划专业名带「（包含专业：…）（XX校区）」尾注，录取分数表用裸名——截断以挂接。
...
// ParseYiFenYiDuan 解析四川一分一段 xlsx（单文件含物理/历史，本科批），按 年×科类 分组。
