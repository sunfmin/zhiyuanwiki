...
func TestParseScores(t *testing.T) {
	...
	rows := [][]string{
		header,
		{"2025", "南京大学", "1101", "物理类", "本科批", "人工智能", "（05）", "首选物理", "680", "196"},
		{"2025", "南京大学", "1101", "历史类", "本科批", "汉语言文学", "（01）", "首选历史", "640", "300"},
		{"2025", "某专科", "9999", "物理类", "专科批", "护理", "", "", "400", "9000"},        // 专科批 → 丢
		{"2025", "南艺", "8801", "艺术类（物理）", "艺术类本科批", "音乐", "", "", "500", "5000"}, // 艺术科类 → 丢
	...
	if got[0].Track != "物理" || got[0].GroupCode != "（05）" || got[0].MinRank != 196 {
...
func TestParsePlan(t *testing.T) {
	...
	rows := [][]string{
		header,
		{"2025", "南京大学", "1101", "物理类", "本科批", "人工智能", "（05）", "首选物理", "30", "四年", "5800"},
...
func TestParseYiFenYiDuan(t *testing.T) {
	...
	rows := [][]string{
		header,
		{"2025", "物理类", "本科批", "463", "683-750", "126", "126"},
		{"2025", "物理类", "本科批", "463", "682", "18", "144"},
		{"2025", "历史类", "本科批", "478", "660", "10", "10"},
		{"2025", "物理类", "专科批", "220", "300", "5", "9999"}, // 专科 → 丢
	}
	got := parseYiFenYiDuan(sheet(t, rows, yfdHeader), "江苏", 2025)
	if len(got) != 2 { // 物理 + 历史
	...
	if wuli.Total() != 144 { // 升序后最低分(682)累计=144
		t.Errorf("Total()=%d, want 144", wuli.Total())
	}
	if r, _ := wuli.ScoreToRank(683); r != 126 { // 683-750 顶段取前导数 683 → 累计 126
		t.Errorf("ScoreToRank(683)=%d, want 126", r)
	}
}
