...
## 省份就绪度（截至 2026-06；已接入 29 省，剩 2 省待数据）
...
- **major · 老文理**（理科/文科，专业平行志愿）：新疆。`provParsers` 里用闭包透传 `group3p12.*With(keep=laowenli={理科,文科})`
  ...
  （原专属 `internal/xj` 编译期包为纯适配器，已收敛进 `provParser` 配置删除，见 ADR-0013 缝在 sheet、ADR-0022 同型收敛。）
- **major · 老文理 · 只有分数**（理科/文科，无位次/无一分一段）：西藏。**考试院不发布一分一段、录取数据无最低位次**
  （学信网全国分数段统计汇总里西藏栏位空白），故新增「只有分数」口径：`provParsers` 里用 `group3p12.ParseScoresScoreOnly`（原专属 `internal/xz` 同上删除）
  保留无位次行（MinRank=0、以最低分数为门槛）；投影按 `PrevRank==0 && PrevScore>0` 走分数域（`majorx` 附 `PrevScore`、
  `dingwei` 以 `s` 键定位、`leaves` 无位次时按最低分去重并记 A/B 分数跨度上界）；前端 `provinces.locatorBasis="score"`
  ——冲稳保按**绝对分差**（`dingwei.classifyScore`，非 V/R 比值），院校/专业页改显最低分走势（`ScoreTrend`），
  全程明示「无位次、分数随难度漂移仅供参考、A 类(少数民族)/B 类(汉族) 两线源未区分」。见 issue #26。
...
**待接入 2 省**：
- **青海 / 山西**（缺数据，见 #26）：本数据源无「含逐专业最低位次的录取分数合表」（仅院校级投档线/PDF）。
  等省考院补齐后按 model 接入，代码侧无需预留。
- **西藏**已改走「只有分数」口径接入（见上「已接入」末条）——它缺的是位次/一分一段（考试院本就不发布），
  而非逐专业录取分数；故无需等数据，已用分数域定位上线。
