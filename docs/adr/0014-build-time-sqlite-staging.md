...
## 省份就绪度（截至 2026-06；已接入 27 省，剩 4 省待数据/待 UI）

**已接入 27 省**（28 个 slug 区其一为浙江独立 major）：
...
**待接入 4 省**：
- **新疆**（数据齐，待 UI，见 #27）：老文理（理科/文科），需专属 `internal/xj`（keep=文理、yfd 无批次列）
  + `trackSlug` 加 理科/文科 + 定位新增 `wenli` 模式（无选科、仅文/理切换）。理科/文科不能进 `group3p12.keep`
  （会污染重庆/贵州等 major 省的老文理历史行）。
